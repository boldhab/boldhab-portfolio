...
const Projects = () => {
  ...
  const getComplexityColor = (complexity: string) => {
    switch (complexity) {
      case "High": return "from-red-500/20 to-orange-500/20 text-red-600 dark:text-red-300 border-red-500/30"
      case "Medium": return "from-yellow-500/20 to-amber-500/20 text-yellow-700 dark:text-yellow-300 border-yellow-500/30"
      default: return "from-green-500/20 to-emerald-500/20 text-green-700 dark:text-green-300 border-green-500/30"
    }
  }

  const getCategoryColor = (category: string) => {
    switch (category) {
      case "Full-Stack": return "from-blue-500/20 to-cyan-500/20 text-blue-700 dark:text-blue-300 border-blue-500/30"
      case "Frontend": return "from-purple-500/20 to-violet-500/20 text-purple-700 dark:text-purple-300 border-purple-500/30"
      case "Backend": return "from-emerald-500/20 to-teal-500/20 text-emerald-700 dark:text-emerald-300 border-emerald-500/30"
      default: return "from-gray-500/20 to-slate-500/20 text-gray-700 dark:text-gray-300 border-gray-500/30"
    }
  }

  return (
    <div id="projects" className="relative min-h-screen w-full py-24 px-4 sm:px-6 lg:px-8 overflow-hidden bg-background transition-colors duration-300">
      ...
      >
        ...
        <div className="text-center mb-24">
          <motion.div
            className="inline-flex items-center gap-2 px-6 py-3 mb-8 rounded-2xl border border-blue-500/30 bg-blue-500/10 backdrop-blur-xl shadow-xl"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
          >
            <Zap className="w-5 h-5 text-blue-500 animate-pulse" />
            <span className="text-sm font-bold text-blue-600 dark:text-blue-300 uppercase tracking-widest">
              Projects
            </span>
          </motion.div>
          ...
          >
            <span className="bg-gradient-to-r from-foreground via-foreground/90 to-blue-500 bg-clip-text text-transparent animate-gradient">
        ...
        >
          {projects.map((project, index) => (
            ...
            >
              ...
              <Card className="relative h-full flex flex-col overflow-hidden bg-secondary/30 border border-border backdrop-blur-xl shadow-2xl group-hover:border-blue-500/40 transition-all duration-500">
                {/* Image Container */}
                <div className="relative h-64 overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-4 flex items-center gap-2">
                    <Badge className="px-3 py-1 text-xs font-semibold rounded-full bg-background/50 text-muted-foreground border-border backdrop-blur-sm">
                ...
                <CardHeader className="pb-4">
                  <CardTitle className="text-foreground text-2xl font-bold group-hover:text-blue-500 transition-all duration-300">
                    {project.title}
                  </CardTitle>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag, i) => (
                      <Badge
                        key={i}
                        variant="outline"
                        className="px-3 py-1.5 text-xs font-medium rounded-full border-border bg-secondary/50 text-muted-foreground hover:bg-blue-500 hover:text-white transition-all duration-300 cursor-default"
                      >
                        {tag}
                      </Badge>
                    ))}
                ...
                <CardFooter className="pt-6 border-t border-border justify-between">
        ...
        >
          <div className="relative">
            ...
            <div className="relative p-12 rounded-2xl bg-gradient-to-br from-blue-500/10 via-transparent to-cyan-500/10 border border-blue-500/30 backdrop-blur-2xl shadow-2xl">
              ...
              <h3 className="text-3xl font-bold text-foreground mb-4">Ready for the Next Project?</h3>
